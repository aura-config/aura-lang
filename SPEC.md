...
| D12 | — (зарезервировано, план v1.3) | Экспорт функций и схем из модулей (`pub def` / `pub type` либо «функции видимы импортёру, но не сериализуются») | Блокирует пакетную экосистему: библиотеки валидаторов, k8s-конструкторы и платформенные стандарты строятся на переиспользуемых функциях. Решение НЕ принято — зафиксирован только вопрос |
...
- Семантика: `.compact()` — удаляет `Null`; `.uniq()` — дедупликация с сохранением первого вхождения; `.merge(other)` — правый перекрывает; `.len()` → `Int`; `.upper()`/`.lower()` — через `str` API; `.first()`/`.last()` — E0317 на пустом списке; `.get(index_or_key, default)` — безопасный доступ (промах → default/Null).
