# Aura — Master Specification интерпретатора (Rust)

Статус: эталон. Любая реализация обязана соответствовать данному документу. Лог дизайн-решений — архитектурный аудит и последующие добавления вроде пакетной экосистемы (D12) и детерминированного времени (D13) — в §0.2.

---

## 0. Общие положения

- Язык: Rust (edition 2021+), MSRV 1.85.
...
### 0.2. Дизайн-решения (нормативные)

| # | Было | Стало | Мотивация |
| --- | --- | --- | --- |
...
| D12 | `def`/`type` всегда приватны | **Принято**: `pub def` / `pub type` попадают в объект модуля — импортёр вызывает `pkg.fn(...)` (приоритет у встроенных методов) и инстанцирует `new pkg.Schema ... end`; из JSON корня pub-элементы верхнего уровня молча исключаются (глубже — E0601); pub не считается мёртвым кодом; **D1×D12**: функция выполняется с capability модуля-происхождения, а не вызывающего; `pub` не перед `def`/`type` — E0206 | Фундамент пакетной экосистемы; явность в духе `shadow`/`new`; изоляция прав не обходится через экспортированные функции |
| D13 | — | **Принято**: `now()`/`timestamp()` не существуют и не появятся — вызов даёт E0533 с подсказкой передать время снаружи (`env("BUILD_TIME", ...)`). Детерминированное время: `"1h30m".parse_duration()` → секунды (Int; единицы d/h/m/s, E0319), `Int.format_duration()`, `"RFC3339".parse_datetime()` → epoch UTC (смещения ±HH:MM, E0320), `Int.format_datetime()` → RFC3339 UTC. Календарная арифметика сверх epoch-Int — территория пакетов | Невоспроизводимый конфиг невозможно написать по построению; длительности и даты покрывают нужды конфигов без болота таймзон в ядре |
| D14 | — | **Принято**: многоветвевое выражение `cond` — `cond (bool -> value)+ else -> value end`. Слева от каждого `->` — `Bool` (иначе E0306, как условие тернарника); справа — любое выражение; `else` обязателен (его отсутствие — parse-ошибка E0207). Побеждает первая истинная ветка. Без деструктуризации значения. | Закрывает пробел на 3+ ветки, где вложенные тернарники нечитаемы; намеренно проще, чем pattern-matching `match` |
| D15 | Каждое поле схемы обязательно | **Принято**: `name: Type = default` делает поле опциональным — при пропуске в `new` дефолт-выражение вычисляется в области инстанцирования (может ссылаться на переменные модуля, напр. `= base + 1`) и вставляется после провайдед-полей; дефолт типизируется как любое значение (E0512). Поле без дефолта по-прежнему обязательно (E0511). Nullable (`?`) полей нет — опциональность не вводит `null`. **Ограничение:** дефолт вычисляется только в области инстанцирования — он не видит соседние поля (`burst: Int = cpu * 2` → E0504); зависимые дефолты — это задача `def`-конструктора (`def quota(cpu) … burst: cpu * 2 … end`). Зависимые дефолты внутри схемы рассмотрены и **отклонены**: фабрика уже даёт и деривацию, и валидацию (`quota = (cpu) -> new Quota  cpu: cpu  burst: cpu * 2 end end`), причём `quota(4)` короче блока `new` — порядок полей и детект циклов не окупились бы. | Закрывает главный пробел схем (реальным конфигам нужны опциональные-с-дефолтом) без нового null: у каждого поля всегда есть значение, форма стабильна |
| D16 | Только однострочные строки в кавычках | **Принято**: блочная строка `text … end` в позиции значения (`key: text` / `x = text` сразу за переводом строки). Даёт обычный `String`, поэтому любое свойство/поле принимает и `"однострочник"`, и блок. Интерполяция `#{…}` и `\`-эскейпы работают точно как в кавычках; общий ведущий отступ срезается, строки соединяются через `\n` (без завершающего перевода строки). Правило терминатора: закрывающий `end` стоит на отступе строки-опенера (`отступ ≤ ключа`); контент обязан быть глубже, поэтому вложенный `end` (Ruby/shell) — это текст (heredoc-коллизия решается отступом). Опенер без `end` — `E0107`. `text` остаётся обычным идентификатором везде, кроме этого контекстного опенера (свойство с именем `text` не затрагивается). Варианта `raw` на старте нет. | Реальные конфиги встраивают короткие скрипты/шаблоны; дуальность inline/блок сохраняет «кавычки = строка» и не требует `exec`/типа-обёртки |
| D17 | `component` неявно вставлял поле `name`; областями видимости были только блоки | **Принято**: `component` удалён, а телом-областью стало каждое тело кода — тела `def` и лямбды принимают statements (`=`, `shadow`, `assert`), как модуль или `domain`. Элемент списка описывается обычными свойствами внутри `map`-лямбды (`name: n` теперь явный), вместе с ним ушёл и блок в позиции выражения. Литерал объекта (`key:` … `end`) остаётся только данными. | Неявный `name` противоречил явности D4/D7 (`new`, `shadow`); функция, не способная назвать подвыражение, была реальным ограничением. Итог: −1 ключевое слово, −1 ветка грамматики, −1 неявное правило |
| D18 | Поле `String` принимает любую строку; закрытый набор проверялся только через `assert` | **Принято**: `enum Name` + члены в кавычках, используется как тип поля схемы (`tier: Tier`). Член — обычный `String`, без типа-обёртки, поэтому JSON не меняется (`"backend"`, а не `{...}`); это *ограничение*, а не преобразование значения. Не-член — `E0514` с подсказкой did-you-mean и списком членов; не-строка — `E0512`. Члены резолвятся **там, где объявлена схема**, поэтому импортированная схема валидируется против enum своего модуля (D12); `pub enum` экспортирует его. Ошибки парсинга: `E0211` (член не строка), `E0212` (дубликат), `E0213` (пусто). | Опечатка `"backand"` доезжала до прода; `assert` ловил её лишь вне схемы, невидимо для инструментов. Сохраняет явность D4/D7 и не добавляет нового вида значений в сериализацию |
| D19 | Хост читает манифест как данные: `eval` пишет JSON, приложение загружает JSON | **Открыто, не принято.** `pub def` манифеста уже является вызываемым значением, а `Interpreter::call_value` уже публичен — значит приложение может вычислить скрипт во время выполнения и звать его на каждый запрос: без промежуточного файла и без пересборки. `crates/aura-lang/examples/scripting.rs` показывает это целиком. Не хватает типа `Script`, который владеет `SourceCache` и прячет времена жизни, JSON на границе ради `serde` и `reload()`. Сознательно не решено обратное направление: скрипт не может позвать хост, потому что `MethodFn` — голый указатель на функцию без захвата состояния, а глобальные встроенные закрыты. Открывать это надо в форме D1 — хост объявляет, что даёт, и `check` это видит, — иначе встраивание теряет то, ради чего затевалось: что `--hermetic` доказывает безвредность скрипта до запуска. **Если D19 делать, инфраструктуру надо назвать заранее, иначе встраивание останется вторым сортом рядом с CLI, проверенным шестью способами:** фаззинг-таргет через `facade::eval_source` (собственная логика фасада — резолвер в памяти, проводка прав, многофайловость — сегодня не покрыта ничем), бенчмарк на `call_value` (стоимость одного вызова правила, горячий путь сценария скриптования, сейчас не измерена), тесты на перезагрузку и времена жизни, сквозная программа-потребитель. | Вычисляемая конфигурация напрямую, без генерации промежуточного файла для приложения |
| D20 | Конфигурация только покидает Aura: `eval --format` пишет JSON/YAML/TOML, а внести существующую нечем | **Открыто, не принято.** Три команды и сознательно не четвёртая. `aura import config.yaml -o config.aura` — механический перевод, зеркало `serialize`, пропущенное через `fmt`, поэтому вывод канонический по построению. Это точка входа, а не результат: получаются плоские данные без `=`, схем и `cond`, и говорить об этом надо прямо — иначе человек сконвертирует, не увидит выигрыша и решит, что языку нечего дать. `aura diff config.aura --against config.yaml` вычисляет и сравнивает как деревья, показывая расходящиеся пути; накопление путей уже есть в `serialize`. Это то, что возможно только благодаря детерминизму, и именно оно делает безопасным любой рефакторинг — хоть руками, хоть ассистентом. `aura check --suggest` докладывает, что улучшить; по умолчанию выключено, чтобы гейт в CI остался чистым. Каждый анализ точен, и каждое предложение несёт своё свидетельство — пути и счётчики, — потому что решает человек: повтор бывает смыслом, а бывает совпадением. **Алгоритмы, все детерминированные:** повторяющиеся литералы — хеш «значение → список путей»; повторяющиеся формы — группировка объектов по хешу отсортированного множества ключей; почти одинаковые формы — мера Жаккара на множествах ключей, где отличие в одном поле ложится на необязательное поле со значением по умолчанию из D15; различия окружений — попарный диф по дереву между соседями одной формы, это и есть кандидат в `cond`. Ранжирование — по сокращению длины описания: сэкономленные байты против стоимости объявления. Если однажды понадобятся повторяющиеся *узоры вложенности*, а не формы, — детерминированный линейный ответ там даёт грамматический вывод (Sequitur). Новых зависимостей нет. **Явно отклонено: `aura optimize`.** Инструменту, который сам переписывает чужой конфиг, доверять не будут, а на выходе — совет, и его место в `check` рядом с существующими W05xx. **И `aura convert`** — это `eval --format` плюс `import` под третьим именем. **Не байесовский, намеренно:** скрытого параметра, который надо оценивать, здесь нет. Сколько раз встречается литерал — считают, а не оценивают, и апостериорную вероятность читатель не проверит так, как проверяет список путей. | Adoption: барьер входа в любой конфигурационный язык — уже существующие четыре тысячи строк YAML и час на их перепечатывание |
| D21 | Поле схемы с типом `List` или `Object` ничего не обещает о содержимом | **Открыто, не принято.** `List<Service>` — тип элемента у поля-коллекции. Показано на 0.1.1: `endpoints: ["not an endpoint", 42, true]` внутри `new Service` проходит под `--strict`, и единственная жалоба — что тип `Endpoint` не используется. Обходной путь есть — собирать каждый элемент через `new` внутри `.map`, и он действительно валидирует. Но это дисциплина в каждом месте сборки, а не гарантия в объявлении: ничто её не требует, поздний рефакторинг тихо её снимет, а схема по-прежнему не сообщает читателю, что лежит в списке. Четыре следствия закрытия: мусор отвергается с путём (`cluster.services[0].endpoints[1]`); схема становится самодостаточной для чтения; инварианты по коллекции можно писать о данных, форму которых автор не собирал лично; и `aura types` выдаёт `Vec<Service>` вместо `Vec<serde_json::Value>` — сегодня самая частая форма конфигурации приезжает в хост нетипизированной. Синтаксис не выбран. | Список объектов — самая частая форма реальной конфигурации и обслужена хуже всего; а пока типа нет, обходной путь твердеет в идиому, которую 1.0 придётся сохранять |
| D22 | `github/<owner>/<repo>@vX.Y.Z` резолвится в `package.aura` в корне репозитория, поэтому один репозиторий — один пакет, и найти пакет нечем | **Открыто, не принято.** Репозиторий-каталог с правилами публикации и указателем — причём **проверяющий, а не курирующий**: CI берёт каждый пакет из списка и прогоняет `check --strict --hermetic`, `fmt --check` и вычисление с закреплённым локом, так что список не может утверждать то, чего пакеты не выполняют. Обнаружение — ещё и через тему на GitHub. **Реестра-сервиса нет.** `aura.lock` уже несёт хеш целостности потока токенов, поэтому реестр добавил бы точку доверия, без которой установка и так безопасна. Опционально расширить резолвер до `github/<owner>/<repo>/<subdir>`, чтобы официальный набор жил в одном репозитории, а сторонние пакеты сохраняли по репозиторию на пакет. Плата — общий тег: правка одного пакета двигает версию всем. Поддержать обе формы, а не заменять одну другой. **Правила для сообщества намеренно лёгкие, и причина — модель прав.** Импортированный модуль не может читать файлы и окружение, даже когда право есть у корня (D1), а экспортированная функция выполняется с правами модуля-происхождения (D12) — то есть худшее, чего добьётся враждебный пакет, это неверное значение, а не утечка или исполнение. Сравните с npm или PyPI, где пакет исполняет код при установке. Поэтому правила покрывают только то, чего язык не проверяет сам: точный тег `vX.Y.Z`, `package.aura` в корне, `pub` только на API, README с назначением и обещание не ломать минорной версией. Первые пакеты выращивать из примеров, доказавших пользу, а не придумывать — набор, написанный до спроса, это сопровождение без подтверждённых пользователей. | Adoption требует обнаружения, а модель прав позволяет получить его, не строя реестр и не доверяя ему |
...
- **Блочные строки (D16):** голый `text` в позиции значения (предыдущий токен `:` или `=`) с переводом строки следом открывает многострочную строку, захватываемую сырьём до одинокого `end`, чей отступ `≤` отступа строки-опенера. Общий ведущий отступ срезается; строки соединяются через `\n` (без завершающего перевода строки); интерполяция `#{…}` и `\`-эскейпы работают как выше, так что токен — обычный `Str`/`InterpStr` (разделители-переводы строк — реальные срезы `\n`, что сохраняет zero-copy). Более глубокий `end` — это контент (heredoc-коллизия решается отступом); отсутствие терминатора — `E0107`. Вне этого контекстного опенера `text` остаётся обычным идентификатором.

### 2.4. Ключевые слова, идентификаторы, пути импорта

- Идентификатор: `[a-zA-Z_][a-zA-Z0-9_]*`, затем сверка с таблицей ключевых слов (`import as type enum def end domain new assert shadow true false null`).
...
pub enum Stmt<'a> {
    ...
    Property { key: &'a str, value: Expr<'a>, span: Span },              // key: expr | key: <объектный блок>
...
- Вложенный объект: `key:` + непосредственный `Newline` (значения на строке нет) открывает `ObjectLiteral` до `end`. Это единственная блочная форма объекта (D3); ранее существовавшая инлайн-форма `metrics port: 9090 ...` — синтаксическая ошибка `E0201` с подсказкой «use `metrics:` … `end`».
...
- Ссылки на данные идут строго вверх, поэтому графы значений не образуют циклов.
  Замыкание функции замкнуло бы цикл на своё определяющее окружение, поэтому
  замыкания держат `Weak`-ссылку; арена окружений интерпретатора владеет каждым
  окружением на время вычисления и затем аккуратно дропает весь DAG. Утечки нет.

Новые фреймы: тело `domain`, `def`, лямбда, каждый вызов `map`-колбэка (D17: все тела кода — области видимости).
...
- Форматы (D11-пакет): `.parse_toml()` / `.parse_json()` / `.parse_yaml()` на `String` → `Value` (целые → `Int`, единый маппинг через serde_json); `.to_json()` / `.to_yaml()` / `.to_toml()` на `Object`/`List` → `String` (E0603: TOML требует объект на верхнем уровне и не имеет null). Эмиттеры разделяются с CLI `--format json|json-flat|yaml|toml`.
- Время (D13, детерминированное): `.parse_duration()` на `String` → `Int` секунд (единицы d/h/m/s, E0319 на ошибке); `.format_duration()` на `Int` → `String`; `.parse_datetime()` на `String` (RFC3339) → `Int` epoch UTC (смещения `±HH:MM`, E0320 на ошибке); `.format_datetime()` на `Int` → `String` (RFC3339 UTC). `now()`/`timestamp()` не существуют (E0533).
- Расширение stdlib: `String` — `.trim()`, `.split(sep)` (непустой `sep` → List), `.replace(from, to)`, `.starts_with(p)`/`.ends_with(s)` → Bool, `.to_int()`/`.to_float()` (E0314 при ошибке); `List` — `.sort()` (по возрастанию, взаимно сравнимые скаляры, иначе E0306), `.reverse()`, `.sum()` (Int, либо Float при наличии Float; E0304 при переполнении Int), `.min()`/`.max()` (E0317 на пустом), `.flatten()` (один уровень: разворачивает вложенные списки, скаляры сохраняет), `.slice(start, end)` (полуоткрытый, индексы обрезаются по границам); `Int`/`Float` — `.abs()`; `Int`/`Float`/`Bool`/`String` — `.to_str()` (то же представление, что в интерполяции). Дайджесты и кодеки на `String` (чистые функции — без I/O и часов, поэтому D1/D13 сохраняются): `.sha256()` → hex в нижнем регистре, `.base64()` → стандартный base64 с паддингом, `.base64_decode()` → String (E0321, если не валидный base64 или не UTF-8).
...
Результат модуля — `Value::Object` только из свойств `key:` и `domain`-блоков (ключ = label); `=`-биндинги, `def`, `type` — приватны (D10). Тело блока экспортирует свои свойства и вложенные блоки по тому же правилу. Тела `def` и лямбды следуют ему же (D17): `key:` — поле результирующего объекта, `=` остаётся приватным.
...
- `new Schema ... end`: отсутствующее поле → `E0511`; несоответствие типа (`Int` vs `String`, `Int` vs `Float` — тоже несоответствие) → `E0512` — всегда ошибки. Лишнее поле → `E0513` в `--strict`, иначе предупреждение (поле сохраняется).
...
`aura fmt` — канонический форматтер, меняющий только пробелы. Он: (1) нормализует
отступы (2 пробела/уровень по токен-глубине: `domain`/`def`/`type`/`new`/
`->`/`[`/`(`/`key:`-в-конце открывают, `end`/`]`/`)` закрывают; строки-продолжения
после `,`/`=`/оператора — +1); (2) канонизирует внутристрочные пробелы (последовательности
пробелов/табов между токенами схлопываются до одного; строки и хвостовые комментарии не
трогаются); (3) колоночно выравнивает подряд идущие `name = value`, `key: value` и арки
`cond` (`cond -> value`) вместе с хвостовыми `# комментариями` — пустая строка, строка-комментарий,
другой вид конструкции или смена отступа разрывают прогон, а арка `else` не выравнивается.
...
| --- | --- | --- |
| 1 | `lexer` | Токенизация эталонного манифеста; snapshot-тест; property-тест «конкатенация span'ов = исходник»; тесты D2 (`[a \n -b]` → 2 элемента) и D8 (`E0104` без версии) |
