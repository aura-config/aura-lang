...
fn add_then_eval_offline() {
    ...
    // `aura1-` is the token-stream hash; a `sha256-` byte hash is only written by
    // versions before that change, and is still accepted when reading.
    assert!(
        lock.contains("pkg/validators") && lock.contains("aura1-"),
        "{lock}"
    );
...
/// A lock written before token-stream hashing must keep working. Failing an old
/// lock because the algorithm changed would be indistinguishable from reporting
/// real tampering, which is the one thing an integrity check must never do.
#[test]
fn a_legacy_lock_still_verifies_and_then_upgrades() {
    let work = std::env::temp_dir().join(format!("aura-legacy-lock-{}", std::process::id()));
    let registry = work.join("registry");
    std::fs::create_dir_all(&work).unwrap();
    ...
    // Rewrite the lock the way an older version would have: a byte hash.
    let pkg_text = std::fs::read_to_string(registry.join("pkg/validators/1.0.0.aura")).unwrap();
    let legacy = aura_lang::vfs::lockfile::legacy_integrity_of(&pkg_text);
    assert!(legacy.starts_with("sha256-"));
    std::fs::write(
        work.join("aura.lock"),
        format!(
            "# Generated by aura. Do not edit manually.

[modules.\"pkg/validators\"]
version = \"1.0.0\"
integrity = \"{legacy}\"
"
        ),
...
",
    ...
    // Under --frozen the legacy entry verifies and the lock is left alone.
    let run = aura(
        &work,
        &[
            "eval",
    ...
    assert_eq!(run.code, 0, "a legacy lock must verify: {}", run.stderr);
    let after_frozen = std::fs::read_to_string(work.join("aura.lock")).unwrap();
    assert!(
        after_frozen.contains("sha256-"),
        "--frozen must not rewrite the lock: {after_frozen}"
    );

    // Without --frozen the same run upgrades the entry to the new algorithm.
    let run = aura(
        &work,
        &[
            "eval",
    ...
    let upgraded = std::fs::read_to_string(work.join("aura.lock")).unwrap();
    assert!(
        upgraded.contains("aura1-") && !upgraded.contains("sha256-"),
        "the entry should have been upgraded: {upgraded}"
    );

    // And the payoff: reformatting the package no longer breaks the lock.
    let reformatted = format!(
        "# a comment added later

{pkg_text}"
    );
    std::fs::write(registry.join("pkg/validators/1.0.0.aura"), &reformatted).unwrap();
    let run = aura(
        &work,
        &[
            "eval",
    ...
    assert_eq!(
        run.code, 0,
        "a comment-only change must not trip the integrity check: {}",
        run.stderr
    );

    // A real change still does.
    std::fs::write(
        registry.join("pkg/validators/1.0.0.aura"),
        pkg_text.replace("65536", "70000"),
    )
    .unwrap();
    let run = aura(
        &work,
    ...
    assert_eq!(run.code, 1, "a changed constant must be caught");
    assert!(
