...
fn cyclic_import_reports_full_chain() {
    ...
    let err = loader.eval_entry(&mut it, &ImportSpec::File("a.aura")).unwrap_err();
    assert_eq!(err.code, "E0401");
    assert!(err.message.contains("a.aura -> b.aura -> a.aura"), "chain missing: {}", err.message);
}
...
fn shared_module_is_loaded_once() {
    ...
    let mut loader = Loader::new(&cache, &resolver);
    let mut it = Interpreter::new(Options::default());
    let v = loader.eval_entry(&mut it, &ImportSpec::File("root.aura")).unwrap();
    assert_eq!(get(&v, "x"), Value::Int(23));
    assert_eq!(resolver.loads.borrow()["shared.aura"], 1, "shared module must be loaded exactly once");
}
...
fn imported_modules_have_no_io_capability_d1() {
    ...
    let err = loader.eval_entry(&mut it, &ImportSpec::File("root.aura")).unwrap_err();
    ...
    it2.env_overrides.insert("HOME".to_string(), "/home/x".to_string());
    let v = loader2.eval_entry(&mut it2, &ImportSpec::File("root.aura")).unwrap();
...
fn lockfile_integrity_and_frozen() {
    ...
    loader2.lock.entries.insert(
        "pkg/lib".to_string(),
        LockEntry { version: "1.2".to_string(), integrity: "sha256-deadbeef".to_string() },
    );
    let mut it2 = Interpreter::new(Options::default());
    let err = loader2.eval_entry(&mut it2, &ImportSpec::File("root.aura")).unwrap_err();
    ...
    let err = loader3.eval_entry(&mut it3, &ImportSpec::File("root.aura")).unwrap_err();
...
fn manifest_end_to_end_through_vfs() {
    let resolver = mem(&[
        ...
        ("templates/k8s_defaults.aura", "global_labels:\n  team: \"core\"\nend"),
    ...
    assert_eq!(loader.lock.entries["github/actions/rust-cache"].version, "1.2");
}
