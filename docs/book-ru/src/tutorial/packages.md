...
и фиксируется в `aura.lock` с хешем целостности. `eval` всегда работает оффлайн.

## aura.lock и --frozen

- лок хранит точную версию + integrity каждого пакета;
- хеш считается по **потоку токенов**, а не по байтам: переформатирование пакета
  или исправление опечатки в комментарии проверку не срабатывают, а переименование
  поля или изменение числа — срабатывает. Проверка целостности, которая срабатывает
  на безобидных правках, приучает обновлять лок не глядя, что её и обесценивает;
- запись `sha256-` от старой версии по-прежнему проверяется и обновляется при
  первой возможности записи;
