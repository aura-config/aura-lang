...
### Набросок: правила, которые приложение исполняет, а не только читает

`eval_file` возвращает JSON, а JSON не умеет хранить функцию. Но `pub def` в Aura
— это значение, и интерпретатор умеет его вызвать: скрипт объявляет
`route(path, method)`, приложение зовёт его на каждый запрос, и поведение
меняется правкой файла, а не пересборкой.

`cargo run --example scripting` в этом репозитории показывает это целиком.

Помечено как набросок сознательно. Составные части публичны и работают уже
сейчас, но добраться до них можно только спустившись ниже `facade` и удерживая
`SourceCache` вручную, а сам скрипт по-прежнему не может позвать приложение
обратно. Закрывать второй пробел нужно в форме модели прав, а не сбоку от неё:
произвольные функции хоста отдали бы ровно то свойство, ради которого Aura стоит
встраивать, — что `aura check --hermetic` доказывает безвредность скрипта до
запуска.
