...
| E0108 | разделитель разрядов стоит неверно | `_` группирует по три от точки: `10_000_000`, `0.000_001` — но не `100_`, `1__0` и не `2_3_7_1_9_3_3` |
...
| E0322 | склеенный список или строка превышает предел размера | `+` удваивает: потолок 1M элементов / 16 MiB |
| E0323 | `to_object()` получил один и тот же ключ дважды | молчаливая перезапись зависела бы от порядка элементов; уберите дубли заранее |
...
| E0515 | инвариант схемы не выполняется для этого экземпляра | собственный `assert` схемы; показано его же сообщение |
| W0512 | эффектный вызов в импортированном модуле | у импортов нет I/O-прав; пересмотрите дизайн пакета |
| W0513 | нулевое поле заполняется из `get(key)` или `env(name)` без значения по умолчанию | отсутствие тихо станет `null`; задайте значение по умолчанию или уберите `?` |
